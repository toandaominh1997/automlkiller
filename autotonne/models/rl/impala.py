...
@ModelFactory.register('rl-impala')
class IMPALAContainer(object):
    def __init__(self, **kwargs):
        super().__init__()
        self.estimator = IMPALArl(**kwargs)

class IMPALArl(object):
    def __init__(self, env, env_config, config):
        self.config = config
        self.config['env_config'] = env_config
        self.env = env(env_config)
        self.agent = IMPALATrainer(config = self.config, env = env)
