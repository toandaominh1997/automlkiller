...
from ray.rllib.agents.mbmpo import MBMPOTrainer
...
@ModelFactory.register('rl-mbmpo')
class MBMPOContainer(object):
    def __init__(self, **kwargs):
        super().__init__()
        self.estimator = MBMPOrl(**kwargs)

class MBMPOrl(object):
    def __init__(self, env, env_config, config):
        self.config = config
        self.config['env_config'] = env_config
        self.env = env(env_config)
        self.agent = MBMPOTrainer(config = self.config, env = env)
